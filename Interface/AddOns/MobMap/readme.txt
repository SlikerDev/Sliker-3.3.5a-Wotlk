...
v3.54:
- fixed:   The query buttons in the standard quest log now correctly search for items, pickups
           etc. again instead of only searching for mobs
- fixed:   Some very specific display issues with the query button results
- changed: The minimap button behavior has been changed in order to allow mods like SexyMap or
           MinimapButtonBag (MBB) to capture and manage the MobMap minimap button
- fixed:   Placement issues of the MobMap buttons on the World Map since WoW Patch 3.3.3
- fixed:   Placement issues of the MobMap dots on the World Map since WoW Patch 3.3.3

v3.55:
- fixed:   Removed some seldomly called debug code that was left from fixing a bug in 3.54