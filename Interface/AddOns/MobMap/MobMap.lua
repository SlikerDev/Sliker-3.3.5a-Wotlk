-- MobMap - an ingame mob position database - v3.55
-- main code file
...
function MobMap_ParseQuestObjective(objective, typeID, objectivetext)
	...
	if(filtered==nil) then
		filtered=string.match(objective, "^(.*): %d+/%d+$");
		if(filtered==nil) then
			filtered=objective;
		end
	end
...
function MobMap_PlaceMobMapButtonFrame()
	...
	if(mobmap_button_position==2) then
		if(WORLDMAP_SETTINGS.size == WORLDMAP_WINDOWED_SIZE) then
			MobMapButtonFrame:SetPoint("TOPRIGHT","WorldMapPositioningGuide","TOPRIGHT",-68,-14);
		else
			MobMapButtonFrame:SetPoint("TOPRIGHT","WorldMapPositioningGuide","TOPRIGHT",-44,-1);
		end
		...
		MobMapCheckButton:SetPoint("TOPRIGHT","MobMapButtonFrame","TOPRIGHT",-24,-2);
		MobMapButtonFrameCurrentMobFrame:ClearAllPoints();
		MobMapButtonFrameCurrentMobFrame:SetPoint("TOPRIGHT","MobMapCheckButton","TOPLEFT",-6,-0);
	...
	if(mobmap_button_position==1) then
		if(WORLDMAP_SETTINGS.size == WORLDMAP_WINDOWED_SIZE) then
			MobMapButtonFrame:SetPoint("TOPLEFT","WorldMapPositioningGuide","TOPLEFT",12,-14);
		else
			MobMapButtonFrame:SetPoint("TOPLEFT","WorldMapPositioningGuide","TOPLEFT",0,-2);
		end
		MobMapButtonFrame:SetFrameLevel(WorldMapPositioningGuide:GetFrameLevel()+20);
		MobMapButton:ClearAllPoints();
		MobMapButton:SetPoint("TOPLEFT","MobMapButtonFrame","TOPLEFT",4,0);
		MobMapCheckButton:ClearAllPoints();
	...
	if(mobmap_button_position==3) then
		if(WORLDMAP_SETTINGS.size == WORLDMAP_WINDOWED_SIZE) then
			MobMapButtonFrame:SetPoint("BOTTOMRIGHT","WorldMapPositioningGuide","BOTTOMRIGHT",-22,-7);
		else
			MobMapButtonFrame:SetPoint("BOTTOMRIGHT","WorldMapPositioningGuide","BOTTOMRIGHT",-1,4);
		end
		MobMapButtonFrame:SetFrameLevel(WorldMapPositioningGuide:GetFrameLevel()+20);
		MobMapButton:ClearAllPoints();
		MobMapButton:SetPoint("BOTTOMRIGHT","MobMapButtonFrame","BOTTOMRIGHT",-4,0);
		MobMapCheckButton:ClearAllPoints();
		MobMapCheckButton:SetPoint("RIGHT","MobMapButton","LEFT",0,-2);
		MobMapButtonFrameCurrentMobFrame:ClearAllPoints();
	...
	if(mobmap_button_position==4) then
		if(WORLDMAP_SETTINGS.size == WORLDMAP_WINDOWED_SIZE) then
			MobMapButtonFrame:SetPoint("BOTTOMLEFT","WorldMapPositioningGuide","BOTTOMLEFT",20,-6);
		else
			MobMapButtonFrame:SetPoint("BOTTOMLEFT","WorldMapPositioningGuide","BOTTOMLEFT",0,-2);
		end
		MobMapButtonFrame:SetFrameLevel(WorldMapPositioningGuide:GetFrameLevel()+20);
		MobMapButton:ClearAllPoints();
		MobMapButton:SetPoint("BOTTOMLEFT","MobMapButtonFrame","BOTTOMLEFT",-4,0);
		MobMapCheckButton:ClearAllPoints();
		MobMapCheckButton:SetPoint("LEFT","MobMapButton","RIGHT",0,-2);
		MobMapButtonFrameCurrentMobFrame:ClearAllPoints();
		MobMapButtonFrameCurrentMobFrame:SetPoint("LEFT","MobMapCheckButton","RIGHT",6,0);
		MobMapButtonFrameCurrentMob:SetJustifyH("left");
		MobMapCheckButton:SetChecked(mobmap_enabled);
