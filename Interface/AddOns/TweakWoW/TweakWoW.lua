...
local ConfigWTFName = {}
	ConfigWTFName[1] = "componentTextureLevel"
	ConfigWTFName[2] = "DesktopGamma"
	ConfigWTFName[3] = "environmentDetail"
	ConfigWTFName[4] = "extShadowQuality"
	ConfigWTFName[5] = "farclip"
	ConfigWTFName[6] = "Gamma"
	ConfigWTFName[7] = "groundEffectDensity"
	ConfigWTFName[8] = "groundEffectDist"
	ConfigWTFName[9] = "gxMultisample"
	ConfigWTFName[10] = "gxMultisampleQuality"
	ConfigWTFName[11] = "gxRefresh"
	ConfigWTFName[12] = "gxResolution"
	ConfigWTFName[13] = "gxTripleBuffer"
	ConfigWTFName[14] = "gxWindow"
	ConfigWTFName[15] = "gxtextureCacheSize"
	ConfigWTFName[16] = "M2Faster"
	ConfigWTFName[17] = "maxFPS"
	ConfigWTFName[18] = "windowResizeLock" 
	ConfigWTFName[19] = "shadowLevel"
	ConfigWTFName[20] = "SkyCloudLOD"
	ConfigWTFName[21] = "specular"
	ConfigWTFName[22] = "textureCacheSize"
	ConfigWTFName[23] = "textureFilteringMode"
	ConfigWTFName[24] = "weatherDensity"
	ConfigWTFName[25] = "particledensity"
	ConfigWTFName[26] = "baseMip"
	ConfigWTFName[27] = "ffxGlow"
	ConfigWTFName[28] = "ffxDeath"
	ConfigWTFName[29] = "gxVSync"
	ConfigWTFName[30] = "gxCursor"
	ConfigWTFName[31] = "gxFixLag"
	ConfigWTFName[32] = "gxMaximize"
	ConfigWTFName[33] = "projectedTextures"
	ConfigWTFName[34] = "farclipoverride"
	ConfigWTFName[35] = "objectFade"
	ConfigWTFName[36] = "characterAmbient" -- TO DO
	ConfigWTFName[37] = "spellEffectLevel"
	ConfigWTFName[38] = "shadowLOD"
	ConfigWTFName[39] = "mapShadows"
	ConfigWTFName[40] = "maxFPSbk"
	
local LibSimpleOptions = LibStub("LibSimpleOptions-1.0")

-- Custom

local function numberToMask(number)
	return tonumber(string.rep("1", number), 2)
end

local function maskToDecimal(mask)
	local tmp = {}
	while mask > 0 do
		local rest = math.fmod(mask, 2)
		table.insert(tmp, rest)
		mask = (mask - rest) / 2
	end
	return string.len(tostring(table.concat(tmp)))
end
...
local function Options(self, anchor)
	...
	local TweakWowGraphicsResetButton = self:MakeButton(
	    'name', TWEAKWOW_SWITCHBUTTON,
	    'description', TWEAKWOW_SWITCHBUTTON_DESCRIPTION,
	    'func', function() SwitchGraphic() 
		end)
	TweakWowGraphicsResetButton:SetPoint("TOPLEFT", subText, "BOTTOMLEFT", 0, -25)	
...
local function OptionsGraphics(self, anchor)
	...
	local TweakWowGraphicsSlider1 = self:MakeSlider(	-- [1]
	    'name', TWEAKWOW_FARCLIPOVERRIDE_TITLE, 
	    'description', TWEAKWOW_FARCLIPOVERRIDE_DESCRIPTION,
	    'minText', '0',
	    'maxText', '1',
	    'minValue', 0,
	    'maxValue', 1,
	    'step', 1,
	    'default', 0,
		'getFunc', function() value = GetCVar("farclipoverride")+0 return GetCVar("farclipoverride")+0 end,
	    'setFunc', function(value) SetCVar("farclipoverride", value) end,
	    'currentTextFunc', function(value) return ("%.1f"):format(value) 
	end)
	TweakWowGraphicsSlider1:SetPoint("TOPLEFT", subText, "BOTTOMLEFT", 0, -25)	
	...
	TweakWowGraphicsSlider2:SetPoint("TOPLEFT", TweakWowGraphicsSlider1, "BOTTOMLEFT", 0, -25)	
	...
	local TweakWowGraphicsSlider3 = self:MakeSlider(	-- [3]
	    ...
	    'currentTextFunc', function(value) 
							if GetCVar("gxAPI") == "D3D9ex" or GetCVar("gxAPI") == "d3d9ex" or GetCVar("gxAPI") == "d3d9EX" or GetCVar("gxAPI") == "D3D9EX" or GetCVar("gxAPI") == "D3D9Ex" then							
								if value == 0 then
									return ("Auto")
								else
	...
	local TweakWowGraphicsSlider4 = self:MakeSlider(	-- [4]
	    'name', TWEAKWOW_TEXTURECACHESIZE_TITLE,
	    'description', TWEAKWOW_TEXTURECACHESIZE_DESCRIPTION,
	    'minText', '0 MB',
	    'maxText', '512 MB',
	    'minValue', 0,
	    'maxValue', 512,
		...
		'getFunc', function(value) 
						if GetCVar("gxAPI") == "D3D9" or GetCVar("gxAPI") == "d3d9" or GetCVar("gxAPI") == "OPENGL" or GetCVar("gxAPI") == "opengl" or GetCVar("gxAPI") == "OpenGL" then
							value = GetCVar("textureCacheSize")+0 return GetCVar("textureCacheSize")+0	
						else
							value = 0 return 0
						end
					end,
	    'setFunc', function(value) 
						if GetCVar("gxAPI") == "D3D9" or GetCVar("gxAPI") == "d3d9" or GetCVar("gxAPI") == "OPENGL" or GetCVar("gxAPI") == "opengl" or GetCVar("gxAPI") == "OpenGL" then
							SetCVar("textureCacheSize", value)
						else
						end
					end,
	    'currentTextFunc', function(value) 
	...
	TweakWowGraphicsSlider4:SetPoint("TOPLEFT", TweakWowGraphicsSlider3, "BOTTOMLEFT", 0, -25)	
	...
	TweakWowGraphicsSlider5:SetPoint("TOPLEFT", TweakWowGraphicsSlider4, "BOTTOMLEFT", 0, -25)	
	...
	local TweakWowGraphicsSlider7 = self:MakeSlider(	-- [10]
	    'name', TWEAKWOW_SPELLEFFECTLEVEL_TITLE,
	    'description', TWEAKWOW_SPELLEFFECTLEVEL_DESCRIPTION,
	    'minText', '9',
	    'maxText', '2000',
	    'minValue', 10,
	    'maxValue', 2000,
	    'step', 10,
	    'default', 10,
		'getFunc', function(value) value = GetCVar("spellEffectLevel")+0 return GetCVar("spellEffectLevel")+0 end,	
	    'setFunc', function(value) SetCVar("spellEffectLevel", value) end,
	    'currentTextFunc', function(value) return ("%.1f"):format(value) 
	end)
	TweakWowGraphicsSlider7:SetPoint("TOPLEFT", TweakWowGraphicsSlider6, "BOTTOMLEFT", 0, -25)	
	...
	local TweakWowGraphicsSlider8 = self:MakeDropDown(	-- [TODO]
	    ...
	    'setFunc', function(value)
			if value == 'd3d9' or value == 'd3d9ex' or value == 'opengl' then 
	...
	TweakWowGraphicsSlider8:SetPoint("TOPLEFT", TweakWowGraphicsSlider7, "BOTTOMLEFT", 0, -25)	
	...
	local TweakWowGraphicsSliderb5 = self:MakeSlider(	-- [15]
	    'name', TWEAKWOW_MAPSHADOWS_TITLE,
	    'description', TWEAKWOW_MAPSHADOWS_DESCRIPTION,
	    'minText', '0',
	    'maxText', '1',
	    'minValue', 0,
	    'maxValue', 1,
	    'step', 1,
	    'default', 1,
		'getFunc', function(value) value = GetCVar("mapShadows")+0 return GetCVar("mapShadows")+0 end,	
	    'setFunc', function(value) 
			oldvalue = GetCVar("mapShadows")+0
			SetCVar("mapShadows", value) 
	...
	TweakWowGraphicsSliderb5:SetPoint("TOPLEFT", TweakWowGraphicsSliderb4, "BOTTOMLEFT", 0, -25)	
	
	local TweakWowGraphicsSliderb6 = self:MakeSlider(	-- [12]
	    'name', TWEAKWOW_SHADOWLOD_TITLE,
	    'description', TWEAKWOW_SHADOWLOD_DESCRIPTION,
	    'minText', '0',
	    'maxText', '1',
	    'minValue', 0,
	    'maxValue', 1,
	    'step', 1,
	    'default', 1,
		'getFunc', function(value) value = GetCVar("shadowLOD")+0 return GetCVar("shadowLOD")+0 end,	
	    'setFunc', function(value) SetCVar("shadowLOD", value) end,
	    'currentTextFunc', function(value) return ("%.1f"):format(value) 
	end)
	TweakWowGraphicsSliderb6:SetPoint("TOPLEFT", TweakWowGraphicsSliderb5, "BOTTOMLEFT", 0, -25)	

	local TweakWowGraphicsSliderb7 = self:MakeSlider(	-- [13]
	    'name', TWEAKWOW_MAXFPS_TITLE,
	    'description', TWEAKWOW_MAXFPS_DESCRIPTION,
	    'minText', '0',
	    'maxText', '200',
	    'minValue', 0,
	    'maxValue', 200,
	    'step', 5,
	    'default', 0,
		'getFunc', function(value) value = GetCVar("MaxFPS")+0 return GetCVar("MaxFPS")+0 end,	
	    'setFunc', function(value) SetCVar("MaxFPS", value) end,
	    'currentTextFunc', function(value) return ("%.1f"):format(value) 
	end)
	TweakWowGraphicsSliderb7:SetPoint("TOPLEFT", TweakWowGraphicsSliderb6, "BOTTOMLEFT", 0, -25)	

	local TweakWowGraphicsSliderb8 = self:MakeSlider(	-- [16]
	    'name', TWEAKWOW_MAXFPSBK_TITLE,
	    'description', TWEAKWOW_MAXFPSBK_DESCRIPTION,
	    'minText', '0',
	    'maxText', '200',
	    'minValue', 0,
	    'maxValue', 200,
	    'step', 5,
	    'default', 30,
		'getFunc', function(value) value = GetCVar("MaxFPSbk")+0 return GetCVar("MaxFPSbk")+0 end,	
	    'setFunc', function(value) SetCVar("MaxFPSbk", value) end,
	    'currentTextFunc', function(value) return ("%.1f"):format(value) 
	end)
	TweakWowGraphicsSliderb8:SetPoint("TOPLEFT", TweakWowGraphicsSliderb7, "BOTTOMLEFT", 0, -25)	
...
local function OptionsCPU(self, anchor)
	...
	local coresValues = {'0', 'All cores'}
	for i = 1, 32, 1 do
		table.insert(coresValues, tostring(numberToMask(i)))
		table.insert(coresValues, tostring(i) .. " cores")
	end
	...
	local TweakWowCPUSlider1 = self:MakeDropDown(	-- [1]
	    ...
	    'default', '0',
		'getFunc', function(value)
			local mask = tostring(GetCVar("processAffinityMask"))
			return mask or "0"
		end,
	    'setFunc', function(value)
			SetCVar("processAffinityMask", tonumber(value))
		end)
...
function SwitchGraphic(self)
	for count = 1, table.maxn(ConfigWTFName) do
		-- 1st time we run this
		if TweakWoWDB.graphic[table.maxn(ConfigWTFName)] == nil then
			for count = 1, table.maxn(ConfigWTFName) do
				value = GetCVar(ConfigWTFName[count])
				if value == nil then value = "nil" end
				tremove(TweakWoWDB.graphic, count)
				tinsert(TweakWoWDB.graphic, count, value)
			end
		end
		-- Save current
		value = GetCVar(ConfigWTFName[count]) 
		...
		if value == nil then value = "nil" end
		-- just another ugly "horizonfarclip" fix
		if count == 15 then value = "nil" end
		-- Set last saved settings
		if TweakWoWDB.graphic[count] ~= "nil" and TweakWoWDB.graphic[count] ~= nil then 
			SetCVar(ConfigWTFName[count], TweakWoWDB.graphic[count]) 
		end
...
local function OnEvent(self, name)
	...
	-- Initialize Variables (Graphic Switch Button)
